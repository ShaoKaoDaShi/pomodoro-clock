...
const App = () => {
  ...
  const sendNotification = (title: string, body: string) => {
    platform.sendNotification(title, body);
  };
  ...
  useEffect(() => {
    ...
    const listener = platform.onStopFollowMouse(stopFollowMouseHandler);
    return () => {
      platform.offStopFollowMouse(listener);
    };
  }, []);
  ...
  return (
    <div
      className={`flex justify-center items-center min-h-screen w-full overflow-x-hidden select-none bg-gradient-to-br ${theme.bg} transition-colors duration-500`}
    >
      {/* 拖拽区域 */}
      <div
        className={`fixed top-0 left-0 w-full h-8 z-50 [-webkit-app-region:drag] ${isMiniMode || isNative ? "hidden" : ""}`}
        id="drag-region"
      ></div>
        ...
        className={`
          ...
          ${
            isMiniMode
              ? "mini-mode"
              : isNative
                ? "w-full max-w-[340px] m-4 p-8 rounded-3xl shadow-2xl bg-white/95 backdrop-blur-xl border border-white/60"
                : `w-80 p-8 rounded-3xl shadow-2xl bg-white/95 backdrop-blur-xl border border-white/60 bg-gradient-to-br ${theme.bg}`
          }
        `}
      >
        {/* 关闭按钮 (模拟) */}
        <div
          className={`absolute top-4 right-4 flex gap-1 z-50 group ${isMiniMode || isNative ? "hidden" : ""}`}
        >
          <div
            className="w-3 h-3 rounded-full bg-slate-300 group-hover:bg-red-500 cursor-pointer transition-colors duration-200"
            onClick={() => platform.quit()}
          ></div>
        </div>
        ...
        >
          {!isRunning ? (
            <button
              className={`flex-1 py-4 px-6 rounded-2xl text-white font-semibold shadow-lg hover:shadow-xl active:scale-95 transition-all duration-200 ${theme.button}`}
              onClick={startTimer}
            >
              开始
            </button>
            ...
            <button
              className="flex-1 py-4 px-6 rounded-2xl bg-amber-400 text-amber-900 font-semibold shadow-lg shadow-amber-100 hover:bg-amber-300 active:scale-95 transition-all duration-200"
              onClick={pauseTimer}
            >
              暂停
            </button>
          ...
          <button
            className={`w-14 h-14 flex items-center justify-center rounded-2xl transition-all duration-200 active:scale-95 ${theme.buttonSecondary}`}
            onClick={resetTimer}
            title="重置"
          >
            <svg
            ...
            <div className="bg-white/50 p-4 rounded-xl">
              ...
              <div className="flex items-center">
                <input
                  ...
                  min="1"
                  max="60"
                  className={`w-full bg-transparent font-bold text-xl text-gray-700 outline-none p-0 ${theme.text}`}
            ...
            <div className="bg-white/50 p-4 rounded-xl">
              ...
              <div className="flex items-center">
                <input
                  ...
                  min="1"
                  max="30"
                  className={`w-full bg-transparent font-bold text-xl text-gray-700 outline-none p-0 ${isWorkSession ? "text-gray-600" : "text-teal-600"}`}

...
const App = () => {
  ...
  return (
    <div className={`flex justify-center items-center h-screen w-screen overflow-hidden select-none bg-transparent transition-colors duration-500`}>
      ...
      >
        {/* 关闭按钮 (模拟) */}
        <div className={`absolute top-4 right-4 flex gap-1 z-50 group ${isMiniMode ? 'hidden' : ''}`}>
          <div
        ...
        <div className={`text-center mb-6 ${isMiniMode ? 'hidden' : ''}`}>
           <span className={`text-xs font-medium uppercase tracking-widest ${isWorkSession ? 'text-rose-400' : 'text-teal-400'}`}>
        ...
        <div className="text-center relative">
          ...
          <div className={`text-sm font-medium text-gray-500 mb-8 h-6 ${isMiniMode ? 'hidden' : ''}`} id="status-text">
            {isRunning 
              ? (isWorkSession ? "保持专注..." : "享受休息...") 
              : (timeLeft === 0 ? "时间到！" : (timerIntervalRef.current ? "已暂停" : "准备开始"))}
          </div>
        </div>

        {/* Main Controls */}
        <div className={`flex justify-center gap-4 mb-8 controls-area ${isMiniMode ? 'hidden' : ''}`}>
          {!isRunning ? (
            <button
        ...
        `}>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white/50 p-3 rounded-xl">
              <label className="block text-xs font-medium text-gray-500 mb-1">专注时长</label>
            ...
            <div className="bg-white/50 p-3 rounded-xl">
              <label className="block text-xs font-medium text-gray-500 mb-1">休息时长</label>
              <div className="flex items-center">
                <input
                  type="number"
                  value={breakTime}
          ...
          <div className="flex justify-between items-center px-1">
            <label className="flex items-center gap-2 cursor-pointer group">
              ...
              <span className="text-sm font-medium text-gray-600 group-hover:text-gray-800 transition-colors">总在最前</span>
            </label>
